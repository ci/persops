...
- Plan/impl reviews: use `$autoreview` with the global engine/model routing. Explicit owner requests for another review model take precedence.
- Haiku: high-fan-out trivial lookups only; never for anything that ships.
- Mechanics: gpt-5.5 only via Codex CLI (`codex exec` / `codex review`; inspect `~/.codex/config.toml` for current defaults). Investigations: `codex exec -s read-only` with a self-contained prompt, or the codex plugin (codex:rescue skill / codex-rescue agent).
