...
- Full flake updates: re-check temporary upstream workarounds in `flake.nix` and `machines/amalthea.nix` (Darwin `mise` HTTP skip, `python312` out-only) and remove if upstream fixed.
